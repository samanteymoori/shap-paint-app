...
export function getPointMeasures ( point1:Point,point2:Point,point3:any = null ): any{
    const X1 = point1.x;
    const Y1 = point1.y;
    const X2 = point2.x;
    const Y2 = point2.y;
    ...
    const LargerLength = PositiveWidth > PositiveHeight ? PositiveWidth : PositiveHeight;
    return {
            X1,X2,Y1,Y2,
            Width,
            Height,
...
export function calculateBoundRectangle ( rectangle: Rectangle ): void{
    ...
    rectangle.Shape.LowerBoundX = Math.min( rectangle.X1, rectangle.X1 + rectangle.Width );

    rectangle.Shape.UpperBoundX = Math.max( rectangle.X1, rectangle.X1 + rectangle.Width );
    
    rectangle.Shape.LowerBoundY = Math.min( rectangle.Y1, rectangle.Y1 + rectangle.Height );
    
    rectangle.Shape.UpperBoundY = Math.max( rectangle.Y1, rectangle.Y1 + rectangle.Height );
...
export function calculateBoundCircle ( circle: Circle ): void{
    ...
    circle.Shape.LowerBoundX = Math.min( circle.X1 - circle.Radius, circle.X1, circle.X1 + circle.Radius );
    
    circle.Shape.UpperBoundX = Math.max( circle.X1 - circle.Radius, circle.X1, circle.X1 + circle.Radius );
    
    circle.Shape.LowerBoundY = Math.min( circle.Y1 - circle.Radius, circle.Y1, circle.Y1 + circle.Radius );
    
    circle.Shape.UpperBoundY = Math.max( circle.Y1 - circle.Radius, circle.Y1, circle.Y1 + circle.Radius );
    
}
export function createTriangle ( shape: Shape ): any {
    if ( shape.points?.length == 3 )
    {
        const measures = getPointMeasures( shape.points[0], shape.points[1], shape.points[2]  );
...
export function calculateBoundElipsis ( elipsis: Elipsis ): void {
    ...
    elipsis.Shape.LowerBoundX = Math.min( elipsis.X1 - elipsis.RadiusX, elipsis.X1, elipsis.X1 + elipsis.RadiusX );
    
    elipsis.Shape.UpperBoundX = Math.max( elipsis.X1 - elipsis.RadiusX, elipsis.X1, elipsis.X1 + elipsis.RadiusX );
         
    elipsis.Shape.LowerBoundY = Math.min( elipsis.Y1 - elipsis.RadiusY, elipsis.Y1, elipsis.Y1 + elipsis.RadiusY );
         
    elipsis.Shape.UpperBoundY = Math.max( elipsis.Y1 - elipsis.RadiusY, elipsis.Y1, elipsis.Y1 + elipsis.RadiusY );
